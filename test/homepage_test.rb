...
class HomepageTest < Test::Unit::TestCase # rubocop:disable Metrics/ClassLength
  ...
  def test_rate_limiting_blocks_too_many_failed_attempts # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
  ...
  def test_successful_login_clears_rate_limit # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
    # Clear any existing rate limit data
    RateLimiter.clear_attempts('127.0.0.1')
  ...
  def test_session_timeout_middleware_integration # rubocop:disable Metrics/AbcSize
    # Test that the SessionTimeout middleware is loaded and functioning
    # by verifying that sessions work normally (the middleware doesn't break anything)
    ...
    # Basic session functionality should work
    get '/'
    assert last_response.ok?

    # Session should persist across requests (proving middleware doesn't interfere)
    get '/time'
    assert last_response.ok?
    assert last_response.body.include?('Losing your mind already?')
  end
end
